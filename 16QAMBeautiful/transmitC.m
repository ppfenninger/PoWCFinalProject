...
dataBitsRaw = sign(wgn(64*4, 100, 0)); 
dataBits = zeros(64,100);
for m = 1:100
    for n = 1:64
        if isequal(dataBitsRaw((n-1)*4+1:n*4,m), [1;1;1;1])
            dataBits(n,m) = 1+1i;
        elseif isequal(dataBitsRaw((n-1)*4+1:n*4,m), [1;-1;1;1])
            dataBits(n,m) = 1/3+1i;
        elseif isequal(dataBitsRaw((n-1)*4+1:n*4,m), [1;1;1;-1])
            dataBits(n,m) = 1+1/3*1i;
        elseif isequal(dataBitsRaw((n-1)*4+1:n*4,m), [1;-1;1;-1])
            dataBits(n,m) = 1/3+1/3*1i;
